...
class EV(Agent):
    def __init__(self, agent_id, ):
        ...
        self.charge_rate = 0
        self.state_control = 'gone'
        self.actual_state = 'gone'
...
class State:
    def __init__(self, agent, name, eventhandler, localcontroller, transition_dict):
        ...
        reachable_states = self.transition_dict[self]
...
def t_ev_not_charge(y, agent):
    if y == y_max and not agent.state_control == 'gone':
        return True
...
def t_ev_charge(y, agent):
    if y == y_min and not agent.state_control == 'gone':
        return True
